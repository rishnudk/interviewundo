/**
 * MongoDB Seed Validator — seed-mongodb.ts
 *
 * Uses mongodb-memory-server to spin up a real in-process MongoDB instance.
 * Supports Mongo shell syntax (e.g. db.collection.find(filter, proj) / db.collection.aggregate)
 * via a Db Proxy. Converts EJSON dates in input to JS Date objects.
 */
...
import { mongodbProblems } from '../../../src/infrastructure/database/prisma/seed-mongodb';
...
async function executeMongoDB(solutionCode: string, db: Db): Promise<unknown[]> {
  // Create a Proxy for db so db.collectionName works like Mongo shell db.collectionName
  ...
  // Pre-process code to handle await db.createCollection, db.updateMany & db.bulkWrite
  ...
  code = code.replace(/db\.(\w+)\.bulkWrite\(/g, 'await db.$1.bulkWrite(');
  code = code.replace(
    /db\.collection\(["'](\w+)["']\)\.updateMany\(/g,
    'await db.collection("$1").updateMany(',
  );

  // Find the last statement starting with db.
...
async function main() {
  ...
  console.log(`${BOLD}${CYAN}📦  seed-mongodb.ts  (MongoDB Validator)${RESET}`);
  console.log(`${DIM}    ${mongodbProblems.length} problems${RESET}`);
