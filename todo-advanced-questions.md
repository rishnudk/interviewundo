...
- [x] **Modify shared-types enum:** Add `SQL` and `MONGODB` to the `Category` enum in [packages/shared-types/src/enums/index.ts](file:///d:/interview-prep-platform/packages/shared-types/src/enums/index.ts). (`NODEJS` and `REACT` already exist.)
- [x] **Update Prisma schema:** Add `SQL` and `MONGODB` to the `Category` enum in [schema.prisma](file:///d:/interview-prep-platform/apps/backend-api/src/infrastructure/database/prisma/schema.prisma). (`NODEJS` and `REACT` already exist.)
- [x] **Run database migration:** Execute migrations to apply schema changes locally.
- [x] **Seed Node.js problems:** Add Node.js-specific problems with `category: 'NODEJS'` in [seed.ts](file:///d:/interview-prep-platform/apps/backend-api/src/infrastructure/database/prisma/seed.ts). (See problem ideas in Section 5.)
- [x] **Seed React problems:** Add React logic problems with `category: 'REACT'` in seed.ts. (Currently zero REACT problems exist despite the enum value.)
- [x] **Seed SQL problems:** Add SQL query problems with `category: 'SQL'` in seed.ts.
- [x] **Seed MongoDB problems:** Add MongoDB aggregation problems with `category: 'MONGODB'` in seed.ts.
...
- [x] **Update SubmissionWorker routing:** Modify [SubmissionWorker.ts](file:///d:/interview-prep-platform/apps/judge-worker/src/worker/SubmissionWorker.ts) to fetch the problem's `category` and route execution accordingly. Both `JAVASCRIPT` and `NODEJS` categories use the existing `JavascriptExecutor`.
- [x] **Seed Node.js problems:** Problems must use pure functions with Node.js APIs (no HTTP servers, no filesystem reads from disk, no child processes — container has `NetworkDisabled: true` and runs as non-root `node` user).
...
- [x] **Create `infrastructure/docker/` directory** (does not exist yet).
- [x] **Create Dockerfile.sql-runner:** Based on `node:22-slim`, pre-install `better-sqlite3`.
  ```dockerfile
  FROM node:22-slim
  WORKDIR /app
  ...
  USER node
  ```
- [x] **Build the image:** `docker build -t node-sql-runner -f infrastructure/docker/Dockerfile.sql-runner .`
- [x] **Implement SqlExecutor:** Create [apps/judge-worker/src/executor/SqlExecutor.ts](file:///d:/interview-prep-platform/apps/judge-worker/src/executor/SqlExecutor.ts).
...
- [x] **Wire into SubmissionWorker:** Route `SQL` category to `SqlExecutor`.
...
- [x] **Decide test approach:** React problems use the existing `JavascriptExecutor` with the standard `input → expectedOutput` JSON format. Problems are designed to test pure logic:
...
- [x] **No new Docker image needed** — runs on `node:22-slim` like JavaScript problems.
- [x] **No JSX in user code** — problems are pure JS/TS functions that represent React logic patterns.
- [x] **Seed React logic problems** in seed.ts.
- [x] **Wire into SubmissionWorker:** Route `REACT` category to the existing `JavascriptExecutor`.
...
- [x] **Add MongoDB to docker-compose.yml:** Add a `mongo:7` service to [infrastructure/docker-compose.yml](file:///d:/interview-prep-platform/infrastructure/docker-compose.yml).
  ```yaml
  mongo:
    image: mongo:7
    ...
    volumes:
      - mongo_data:/data/db
  ```
- [x] **Create Dockerfile.mongodb-runner:** Based on `node:22-slim`, pre-install `mongodb` driver.
  ```dockerfile
  FROM node:22-slim
  WORKDIR /app
  ...
  USER node
  ```
- [x] **Build the image:** `docker build -t node-mongodb-runner -f infrastructure/docker/Dockerfile.mongodb-runner .`
- [x] **Implement MongodbExecutor:** Create [apps/judge-worker/src/executor/MongodbExecutor.ts](file:///d:/interview-prep-platform/apps/judge-worker/src/executor/MongodbExecutor.ts).
...
- [x] **Wire into SubmissionWorker:** Route `MONGODB` category to `MongodbExecutor`.

#### Executor Refactoring (Optional, Recommended)

- [x] **Create an `IExecutor` interface** to standardize all executors:
  ```typescript
  interface IExecutor {
    execute(
      submissionId: string,
  ...
  ```
- [x] **Refactor `JavascriptExecutor`** to implement `IExecutor`.
...
- [x] **Remove freeform language selector** from [page.tsx](<file:///d:/interview-prep-platform/apps/frontend/src/app/(authenticated)/problems/[slug]/page.tsx>). Replace with auto-detection based on `problem.category`:
...
- [x] **Pass category-derived language** to the submission/run API calls instead of the user-selected value.
...
- [x] Create unit tests for `SqlExecutor`.
- [x] Create unit tests for `MongodbExecutor`.
- [x] Test SubmissionWorker category routing logic.
- [x] Verify Node.js problems execute correctly with the existing `JavascriptExecutor`.
- [x] Verify React logic problems execute correctly with the existing `JavascriptExecutor`.
- [x] Validate seed data — ensure test case formats match executor expectations.
- [/] Perform E2E manual test verification for all new categories (In Progress / Ready for user testing).
